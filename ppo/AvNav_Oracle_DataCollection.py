...
def save_episode_to_dataset(ep_data_dict, dataset_path):
    ...
    ep_data_filename = f"{ep_scene_id}-{ep_length}-{timestamp}-{epid}.bz2"
...
def main():
    ...
    config = get_config(config_paths="env_configs/audiogoal_rgb_depth_ss1_oracle_dset.yaml")
    config.defrost()
    ...
    DATASET_DIR_PATH = "AvNav_Oracle_Dataset_v0" # This assumes the directory is already created, on a drive that have enouhg room for all the data.
